...
STOPWORDS = {
    ...
    "be", "been", "has", "have", "had", "this", "that", "these",
    "those", "its", "their", "they", "them", "than", "into", "over",
    "after", "before", "about", "against", "between", "during", "through",
    "up", "down", "out", "off", "more", "most", "new", "now"
}
...
GOOGLE_NEWS_QUERIES = {
    "Bangladesh": [
        "site:businesspostbd.com business OR economy OR banking",
        "site:unb.com.bd business OR economy OR banking",
        "site:en.prothomalo.com business OR economy OR banking",
        "site:tbsnews.net business OR economy OR banking",
        "site:thedailystar.net business OR economy OR banking",
        "site:bdnews24.com business OR economy OR banking",
        "site:thefinancialexpress.com.bd business OR economy OR banking",
        "Bangladesh Bank monetary policy OR reserves OR forex OR banking",
    ],
    "International": [
        "site:reuters.com business OR economy OR markets",
        "site:bloomberg.com economy OR markets",
        "site:aljazeera.com economy OR business",
        "site:apnews.com business OR economy",
    ],
}
...
def exa_gap_fill(
    ...
    # Exa is used only when RSS coverage is thin.
    if existing_count >= max(
        6,
        needed * 3,
    ):
    ...
    if region == "Bangladesh":
        ...
        queries = [
            ...
            "latest Bangladesh DSE BSEC stock market business companies",
            "latest Bangladesh IMF World Bank ADB economy development",
        ...
        queries = [
            ...
            "latest IMF World Bank WTO major economic developments",
        ]
    ...
    for query in queries:
        try:
            ...
            for result in results.results:
                ...
                if not published_dt or not title:
                    continue
    ...
    "properties": {
        "classifications": {
            ...
            "items": {
                ...
                "required": ["index", "score", "important", "reason", "category"],
...
def rank_candidates(candidates, region):
    """Classify and rank all candidates using the BusinessNewsroom V1 editorial prompt."""
    if not candidates:
        return []
    ...
    recent_published = [
        safe_text(x)
        for x in STATE.get("recent_titles", [])[-60:]
        if safe_text(x)
    ]
    ...
    published_block = "\n".join(f"- {title}" for title in recent_published) or "(none)"
    prompt = f"""
You are a news importance classifier for @BusinessNewsroom, a Telegram business/economic news channel covering Bangladeshi business news and international business news.

Evaluate EVERY input item and assign a score from 0 to 10. This is an editorial importance classifier, not an aggressive pre-filter.

AUDIENCE
The audience cares about business/economic news that materially affects Bangladesh's economy, industries, markets, companies, banking, finance, monetary policy, stock markets, trade, exports/imports, RMG, FDI, remittances, government budget/tax, macroeconomic indicators, major M&A/corporate deals, global commodity/energy/currency movements with business impact, and major IMF/World Bank/ADB/WTO developments.
Technology, retail, industrial and multinational-company news counts only when it is a concrete business/market event such as earnings, listing, deal, leadership, regulation, or materially important corporate development.

IMPORTANCE
9-10: exceptional importance, very broad economic/market impact, major policy decision, major banking crisis, landmark trade/financial event, major corporate collapse/deal, or major global market-moving event.
7-8: clearly important and worth a main channel post.
4-6: relevant or interesting but secondary; keep as backup candidates.
0-3: low-value, promotional, repetitive, unsupported, or clearly outside business/economics.

LOW-VALUE STORIES
Routine corporate announcements, minor launches, generic awards, promotional/marketing content, lifestyle features, opinion without a concrete new event, webinars/roundtables, rumors/speculation, and niche content with little business impact should normally score 0-3 unless the underlying event is genuinely major.

ALREADY PUBLISHED
These are recently published headlines:
{published_block}
If an item is essentially the same event, deal, earnings report, policy change, or development already published, score it 0-3. Do not reject a genuinely new material development merely because it concerns the same broad topic.

RECENCY
The editorial window is the previous 24 hours. Prefer newer stories when significance is similar. Recency is a supporting signal, not a substitute for importance.

SOURCE AND EVIDENCE
Prefer authoritative, original, or established sources. Judge the actual facts available in the title, excerpt, source, and publication age. Repetition by itself does not make a story important.

IMPORTANT V1 RULES
- Do NOT aggressively filter candidates before ranking.
- Classify EVERY input item.
- Preserve useful 4-6 candidates as recoverable backups.
- No exam, bank-job, BCS, viva, study, or competitive-exam relevance should affect the score.
- Do not manufacture importance.

OUTPUT
Return ONLY valid JSON. Include exactly one classification for every input item and preserve its index.
"""
    try:
        ...
        by_index = {idx: item for idx, item in enumerate(regional, start=1)}
        rows = []
        seen = set()
        for row in data.get("classifications", []):
            ...
            item.update({
                ...
                "topic": canonical_topic(safe_text(row.get("category")), region),
                "rank_reason": safe_text(row.get("reason")),
            })
            rows.append(item)
        for idx, original in by_index.items():
            if idx in seen:
                continue
            item = dict(original)
            item.update({
                ...
                "important": False,
                "category": "",
                "topic": canonical_topic(original.get("topic", ""), region),
                "rank_reason": "Retained as a recoverable fallback candidate.",
            })
            rows.append(item)
        ...
        def sort_key(item):
            ...
            return (-int(item.get("importance_score", 0)), -ts)
        ...
        logger.error("Editorial classification failed for %s: %s", region, exc)
        fallback = []
        for rank, item in enumerate(regional, start=1):
            row = dict(item)
            row.update({
                "importance_score": 5,
                "important": False,
                ...
                "rank_reason": "Recency fallback after classification-service failure.",
            })
            fallback.append(row)
        return fallback
    ...
    for item in ordered:
        ...
        for cluster in clusters:
            ...
            if (
                ...
                and (
                    event_similarity_v04(item, representative) >= 0.88
                    or (same_key and event_similarity_v04(item, representative) >= 0.70)
                )
            ):
                cluster.append(item)
                placed = True
...
# ============================================================
# IMAGE BRANDING: ONLY @BusinessNewsroom
# ============================================================
...
def available_candidates(region):
    candidates = []
    seen = set()
